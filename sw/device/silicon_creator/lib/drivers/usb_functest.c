...
void my_control(usb_setup_data_t *setup) {
  switch (setup->request) {
    ...
    case kTestReqDigest: {
      ...
      usb_ep_transfer(0, &digest, sizeof(digest), kUsbTransferFlagsIn);
      break;
    }
    case kTestReqBulkIn: {
      ...
      usb_ep_transfer(
          setup->index, work_buffer, length,
          kUsbTransferFlagsIn |
    ...
    case kTestReqBulkOut:
      base_printf("BulkOut ep=%u sz=%u\r\n", setup->index,
                  (size_t)setup->value + 1);
      usb_ep_transfer(setup->index, work_buffer, (size_t)setup->value + 1,
...
rom_error_t usb_test(void) {
  ...
  usb_ep_init(1, kUsbEndpointTypeOut, 0x40, handler, NULL);
  usb_ep_init(2, kUsbEndpointTypeIn, 0x40, handler, NULL);
  usb_enable(true);
  LOG_INFO("usb ready");
  while (test_exit != 2) {
    usb_poll();
