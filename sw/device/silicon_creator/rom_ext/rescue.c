...
rom_error_t rescue_protocol(boot_data_t *bootdata,
  ...
  if ((hardened_bool_t)config == kHardenedBoolFalse) {
    ...
    rescue_state.flash_limit = FLASH_CTRL_PARAM_REG_PAGES_PER_BANK;
  } else {
    rescue_state.flash_start =
        (uint32_t)config->start * FLASH_CTRL_PARAM_BYTES_PER_PAGE;
    rescue_state.flash_limit = (uint32_t)(config->start + config->size) *
