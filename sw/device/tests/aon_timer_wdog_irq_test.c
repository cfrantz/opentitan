...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  CHECK_DIF_OK(
      dif_aon_timer_irq_is_pending(&aon_timer, kDifAonTimerIrqWdogTimerBark , &pending));
  if (pending) {
    CHECK_DIF_OK(
      dif_aon_timer_irq_acknowledge(&aon_timer, kDifAonTimerIrqWdogTimerBark));
    CHECK_DIF_OK(
      dif_aon_timer_watchdog_pet(&aon_timer));
    uint32_t time;
...
void pwrmgr_cdc_sync(void) {
  ...
  mmio_region_t pwrmgr =  mmio_region_from_addr(TOP_EARLGREY_PWRMGR_AON_BASE_ADDR);
...
void ottf_external_nmi_handler(uint32_t *exc_info) {
  ...
  // Ack the NMI state.
  mmio_region_t ibex = mmio_region_from_addr(TOP_EARLGREY_RV_CORE_IBEX_CFG_BASE_ADDR);
...
static status_t enter_sleep(void) {
  ...
  // Watchdog bite in 2 seconds.
  TRY(dif_aon_timer_watchdog_start(&aon_timer, aon_freq, aon_freq*2,
  ...
  for(;;) {
      ...
      LOG_INFO("Watchdog count = %u", wdog);
  ...
  //LOG_INFO("Woke from sleep at timer=%u. Went to sleep at %u.", timer_after, timer_before);
  return OK_STATUS();
}
...
static status_t test_init(void) {
  ...
  TRY(dif_rv_plic_irq_set_priority(
      &plic, kTopEarlgreyPlicIrqIdAonTimerAonWdogTimerBark, kDifRvPlicMaxPriority));
  // Set Ibex IRQ priority threshold level
  TRY(dif_rv_plic_target_set_threshold(&plic, kTopEarlgreyPlicTargetIbex0,
                                                kDifRvPlicMinPriority));
  // Enable IRQs in PLIC
  TRY(dif_rv_plic_irq_set_enabled(&plic,
                                           kTopEarlgreyPlicIrqIdAonTimerAonWdogTimerBark,
                                           kTopEarlgreyPlicTargetIbex0, kDifToggleEnabled));


  TRY(dif_pwrmgr_set_request_sources(&pwrmgr,
