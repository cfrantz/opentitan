...
void dfu_transport_data(dfu_ctx_t *ctx, void *data, size_t len,
  ...
  if (flags & kUsbTransactionFlagsIn) {
...
rom_error_t rescue_protocol(boot_data_t *bootdata,
  ...
  spi_device_init(
      /*log2_density=*/24, &kRescueSfdpTable,
      sizeof(kRescueSfdpTable) / sizeof(uint32_t));
    ...
    switch (cmd.opcode) {
      case kSpiDeviceOpcodePageProgram: {
        if (cmd.address == kMailboxAddress) {
          ...
          dfu_protocol_handler(&ctx, 0, kUsbTransactionFlagsSetupData, setup);
        } else {
          ...
          if (length >= ctx.expected_len) {
            ...
            dfu_protocol_handler(&ctx, 0, kUsbTransactionFlagsDone, &length);
            ctx.expected_len = 0;
          }
          spi_device_flash_status_clear();
        }
