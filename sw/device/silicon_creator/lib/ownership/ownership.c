...
static rom_error_t locked_owner_init(boot_data_t *bootdata,
  ...
  HARDENED_RETURN_IF_ERROR(owner_block_flash_apply(config.flash, kBootDataSlotA,
                                                   bootdata->primary_bl0_slot));
  HARDENED_RETURN_IF_ERROR(owner_block_flash_apply(config.flash, kBootDataSlotB,
                                                   bootdata->primary_bl0_slot));
...
static rom_error_t unlocked_init(boot_data_t *bootdata,
                                 owner_application_keyring_t *keyring) {
  owner_config_t config = {0};
  uint32_t secondary = bootdata->primary_bl0_slot == kBootDataSlotA
                           ? kBootDataSlotB
                           : kBootDataSlotA;
...
rom_error_t ownership_init(boot_data_t *bootdata,
                           owner_application_keyring_t *keyring) {
  ...
  switch (bootdata->ownership_state) {
    ...
    default:
      ...
      // does nothing.
      error = test_owner_init(bootdata, keyring);
  }
  return error;
}
