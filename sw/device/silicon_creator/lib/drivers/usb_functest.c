  ...
  switch (setup->request) {
    ...
    case kTestReqBulkIn: {
      ...
      usb_ep_transfer(
          ...
          (length < sizeof(work_buffer) ? kUsbTransactionFlagsShortIn : 0));
      usb_ep_transfer(kUsbDirIn | 0, NULL, 0, 0);
      break;
    }
...
void handler(void *ctx, uint8_t ep, usb_transaction_flags_t flags, void *data) {
  OT_DISCARD(ctx);
  ...
  if (ep != 0) {
    int length = (flags & kUsbTransactionFlagsDone) ? *(int *)data : -1;
...
rom_error_t usb_test(void) {
  set_serialnumber();
  usb_init();
  usb_ep_init(0x00, kUsbEpFlagsControl, 0x40, handler, NULL);
  usb_ep_init(0x01, kUsbEpFlagsBulk, 0x40, handler, NULL);
  usb_ep_init(0x82, kUsbEpFlagsBulk, 0x40, handler, NULL);
  usb_enable(true);
  LOG_INFO("usb ready");
  while (test_exit != 2) {
    usb_poll();
