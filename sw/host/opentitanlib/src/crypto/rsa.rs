...
use rsa::pkcs8::{DecodePrivateKey, EncodePrivateKey, DecodePublicKey};
...
impl RsaPublicKey {
    ...
    pub fn from_pkcs1_der_file<P: Into<PathBuf>>(der_file: P) -> Result<RsaPublicKey> {
        ...
        match rsa::RsaPublicKey::read_pkcs1_der_file(&der_file).or_else(|_| rsa::RsaPublicKey::read_public_key_der_file(&der_file)) {
            Ok(key) => {
                validate_key(&key)?;
                Ok(Self { key })
            }
