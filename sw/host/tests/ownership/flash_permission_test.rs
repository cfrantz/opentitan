...
struct Opts {
    ...
    style: transfer_lib::OwnerConfigKind,
...
impl FlashRegion<'_> {
    ...
    fn find_all<'a>(v: &'a str) -> Result<Vec<FlashRegion<'a>>> {
...
fn flash_permission_test(opts: &Opts, transport: &TransportWrapper) -> Result<()> {
    ...
    assert_eq!(
        region[0],
        FlashRegion("data", 0, 0, 32, "RD-xx-xx-xx-xx-xx", "UN")
    );
    assert_eq!(
        region[1],
        FlashRegion("data", 1, 32, 192, "RD-xx-xx-SC-EC-xx", "UN")
    );
    assert_eq!(
        region[2],
        FlashRegion("data", 2, 224, 32, "RD-WR-ER-xx-xx-HE", "UN")
    );
