...
rom_error_t test_owner_init(
  ...
  memset(owner_page[0].data, 0, sizeof(owner_page[0].data));
  ...
  RETURN_IF_ERROR(owner_block_flash_apply(config.flash, kBootDataSlotA, bootdata->primary_bl0_slot));
  RETURN_IF_ERROR(owner_block_flash_apply(config.flash, kBootDataSlotB, bootdata->primary_bl0_slot));
